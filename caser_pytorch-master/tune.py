...
from evaluation_v2 import evaluate_kpi
from interactions_v2 import Interactions
from utils import *
...
# data arguments
parser.add_argument('--train_root', type=str, default='datasets/ml1m/ml-1mtrain.txt')
parser.add_argument('--valid_root', type=str, default='datasets/ml1m/ml-1mvalid.txt')
parser.add_argument('--test_root', type=str, default='datasets/ml1m/ml-1mtest.txt')
...
def objective(trial):
    config.T=trial.suggest_categorical('T', [1,2,3])
    config.learning_rate=trial.suggest_float('learning_rate', 1e-4, 1e-2, log=True)
    config.l2=trial.suggest_float('l2', 0.0, 0.0001, step=0.00005)
    config.drop= trial.suggest_float('drop', 0.1, 0.5, step=0.1)
    config.d=trial.suggest_categorical('d', [5, 10, 20, 30, 50])
    config.nv=trial.suggest_categorical('nv', [1, 2, 4])
    config.nh=trial.suggest_categorical('nh', [4, 8, 16])
    ...
    NDCG_valid, HT_valid, NDCG_test, HT_test=model.fit(train, valid, test, verbose=True)
    return NDCG_valid
...
class Recommender(object):
    ...
    def fit(self, train, valid, test, verbose=False):
        """
        ...
        for epoch_num in range(start_epoch, self._n_iter):
            ...
            if verbose and (epoch_num + 1) % 10 == 0:
                output_str = "Epoch %d [%.1f s]\tloss=%.4f [%.1f s]" % (epoch_num + 1,
                                                                        ...
                                                                        time() - t2)
                print(output_str)
            if verbose and (epoch_num + 1) % 30 == 0:
                NDCG_valid, HT_valid = evaluate_kpi(self, valid, train, k=10)
                NDCG_test, HT_test =evaluate_kpi(self, test, train, k=10)

                output_str = "Epoch %d [%.1f s]\tloss=%.4f,  " \
                             "NDCG_valid@10=%.4f, HT_valid@10=%.4f, " \
                            "NDCG_test@10=%.4f, HT_test@10=%.4f, [%.1f s]" % (epoch_num + 1,
                                                                                         t2 - t1,
                                                                                         epoch_loss,
                                                                                         NDCG_valid,
                                                                                         HT_valid,
        ...
        return NDCG_valid, HT_valid, NDCG_test, HT_test
